...
function handleWebDAVRequest(req, res, backupPath, metadataDir) {
  ...
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, PROPFIND, MKCOL, MOVE, COPY, OPTIONS, HEAD');
...
function handleMove(req, res, filePath) {
  ...
  const destPath = path.join(backupPath, dest.replace(/^\//, ''));
...
app.post('/api/server/start', async (req, res) => {
  ...
  try {
    const state = await createWebDAVServer(port, false);
    server = true;
    res.json({
      success: true,
      url: state.url,
